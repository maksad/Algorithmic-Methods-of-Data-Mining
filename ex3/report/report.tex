  ...
  \begin{document}
    ...
    \section{Problem. \textit{Clustering given a hierarchy:}}
      ...
      \subsection{
        ...
        As we have seen the algorithm returns only the optimal "tuples" with the optimal number of clusters. Therefore, it will always filter the the tuples that have a bigger variance (see line 17 of the pseudocode) before moving to the next step. For example, suppose that we have $k=3$ and the list of tuples looks like this:
        \begin{minted}[bgcolor=bg,fontsize=\small,autogobble]{text}
      ...
      \subsection{
        ...
        \end{align*}
        We are using that formula for each tuple. The number of times $t$ is calculated equals to $2n-1$. $n$ times for the the leaves and $n-1$ times for the parent nodes. \\
